...
# -IF = dependency file to reformat
#
###############################################################################
...
sub get_arguments
{
	...
	ARG: while (@arguments && $arguments[0] =~ s/^-(?=.)//){
		OPT: for (shift @arguments) {
			s/^CD=// && do { $current_directory=$_;	next ARG; };
...
sub get_dependencies
{
	...
	{
		chop;
		# clean up beginning whitespace
		s/^\s//;
		# clean up : (if exists) and \ (line continuations)
		s/[:]* \\$//;
		...
		else 
		{
			...
			{
				# no path, prepend current directory
				if ($file !~ /\//)
				{
				$file = join '/',$current_directory,$file;
				}
				$dependencies[$dependencies_length++] = &remove_rel_path($file);
