...
sub get_dependencies
{
	...
	############################################################################
	open LIST_FILE, "<" , $filename or die "Could not open $filename";
	...
	while(<LIST_FILE>) 
	{
		...
		if( $_ =~ m/^\$include(.*)/ )
		{
			...
			foreach $include_dir (@include_directories)
			{
				...
				# replaces: $depend_file_path = `$tools_path/find.exe \"$include_dir\" -name \"$include_file\"`;
				# eliminates need for find utility (gnuwin32 find incompatible with cygwin dlls)
				opendir(DIR, "$include_dir") or die "Unable to open $include_dir:$!\n";
				my @names = readdir(DIR) or die "Unable to read $include_dir:$!\n";
				closedir(DIR);
				foreach my $name (@names){
					next if ($name eq ".");
					next if ($name eq "..");
					next if (-d $name);
					if ($name eq "$include_file") {          # is this a file named "$include_file"?
					    $depend_file_path = $include_dir . "/" . $include_file;
					}
				}

				if( defined($depend_file_path) )
				{
					if($depend_file_path =~ m/$include_file/ )
					{
						$dependencies[$dependencies_length++] = remove_rel_path($depend_file_path);
						last;
					}
				}
			}
...
{
	local($option_file, *ARRAY) = @_;
	
	my $x = scalar(@ARRAY);

	open OPTION_FILE, "<" , $option_file or die "Could not open $option_file";
