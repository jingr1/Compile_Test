...
sub get_arguments
{
	...
	ARG: while (@arguments && $arguments[0] =~ s/^-(?=.)//){
		OPT: for (shift @arguments) {
			...
			s/^EF=// && do { $error_filename=$_;	next ARG; };
...
sub write_dependency_file
{
	...
	open ERROR_FILE, ">>", "$error_filename" or die "Could not open $error_filename\n";

	foreach $depend_line ( @dependencies )
	{
		if( length($depend_line) > 0 )
		{
		   if ($depend_line  =~ /error\(dcc:/ ) {
		   	printf ERROR_FILE "%s\n", $depend_line;
		   } else {
			$depend_line =~ s/\n//g;
...
sub get_dependencies
{
  ...
  @dependencies = sort keys ( %dependencies_asoc );
