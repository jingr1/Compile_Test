...
# created_by:       zzssm0
# date_created:     Mon Jun 14 11:28:48 2004
# %derived_by:      zzssm0 %
# %date_modified:   Mon Jun 12 11:59:12 2006 %
# %version:         kok_pt1#2 %
###############################################################################
...
#       ARGV[7] - DEFAULT_NEC_V850_ASM_COMMAND_FILE (if no option file is found)
###############################################################################
...
while (<LST>) {
    ...
    if (/^[A-Z\-]+ [0-9]+[ ]+[0-9]+[ \t]+(\.[iI][nN][cC][lL][uU][dD][eE]|\.[bB][iI][nN][cC][lL][uU][dD][eE])/)
    ...
    elsif (/^[A-Z\-]+ [0-9]+[ ]+[0-9]+[ \t]+#[ ]*[dD][eE][pP][eE][nN][dD][eE][nN][tT]/)
